...
def get_vector_info(joint1, joint2):

    vector = joint1 - joint2
    magnitude = norm(vector)
    theta = np.degrees(np.arccos(vector[2]/magnitude))   #arcos(z/r)
    phi = np.degrees(np.arctan2(vector[1], vector[0]))    #arctan(y/x)

    return magnitude, theta, phi

def save_csv(x1,y1,z1,x2,y2,z2,x3,y3,z3,x4,y4,z4,theta1,phi1,theta2,phi2,theta3,phi3,r3,counter):

    header = [["x1", "y1", "z1", "x2", "y2", "z2", "x3", "y3", "z3", "x4", "y4", "z4", "Theta 1", "Phi 1", "Theta 2", "Phi 2", "Theta 3", "Phi 3", "R 3"]]
    data_frame = [[x1,y1,z1,x2,y2,z2,x3,y3,z3,x4,y4,z4,theta1,phi1,theta2,phi2,theta3,phi3,r3]]

    with open(r'C:\Users\Vapps2007\Documents\TG1\PyKinect2-new\PyKinect2-master\data_open\data_open.csv', 'a+', newline='') as file:
        if counter <= 1:
...
def save_rgb(frame, filename):
    path = r'C:\Users\Vapps2007\Documents\TG1\PyKinect2-new\PyKinect2-master\data_open'
    colourfilename = "\RGB"+ filename +".png"
    full_path = path + colourfilename 
    color_frame = np.reshape(frame, (2073600, 4))
    rgb_frame = color_frame[:,0:3] 
    colourframeR = rgb_frame[:,0]
    colourframeR = np.reshape(colourframeR, (1080, 1920))
    colourframeG = rgb_frame[:,1]
    colourframeG = np.reshape(colourframeG, (1080, 1920))        
    colourframeB = rgb_frame[:,2]
    colourframeB = np.reshape(colourframeB, (1080, 1920))
    framefullcolour = cv2.merge([colourframeR, colourframeG, colourframeB])
    cv2.imwrite(full_path, framefullcolour)
              
def save_csv2(x1,y1,z1,x2,y2,z2,x3,y3,z3,x4,y4,z4,theta1,phi1,theta2,phi2,theta3,phi3,r3,counter):

    header = [["x1", "y1", "z1", "x2", "y2", "z2", "x3", "y3", "z3", "x4", "y4", "z4", "Theta 1", "Phi 1", "Theta 2", "Phi 2", "Theta 3", "Phi 3", "R 3"]]
    data_frame = [[x1,y1,z1,x2,y2,z2,x3,y3,z3,x4,y4,z4,theta1,phi1,theta2,phi2,theta3,phi3,r3]]

    with open(r'C:\Users\Vapps2007\Documents\TG1\PyKinect2-new\PyKinect2-master\data_closed\data_closed.csv', 'a+', newline='') as file:
        if counter <= 1:
            writer = csv.writer(file)
            writer.writerows(header)
            writer.writerows(data_frame)
...
def save_rgb2(frame, filename):
    path = r'C:\Users\Vapps2007\Documents\TG1\PyKinect2-new\PyKinect2-master\data_closed'
    colourfilename = "\RGB"+ filename +".png"
    full_path = path + colourfilename 
    color_frame = np.reshape(frame, (2073600, 4))
    rgb_frame = color_frame[:,0:3] 
